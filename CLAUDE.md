...
to `/blog/drafts/` and the watcher rebuilds all posts on any `posts/` change — see CODE_SMELL. Run
`git config core.hooksPath hooks` once per clone to enable the pre-commit backstop.
